...
namespace abb_retrofill_powerbreak.misc_forms
{
    public partial class abb_reports : Form
    {
        ...
        private void exportToExcel()
        {
            ...
            worksheet.Name = DateTime.Now.ToString("MMddyy") + "_abb_report";
            // storing header part in Excel  
            for (int i = 1; i < data_grid_report.Columns.Count + 1; i++)
            {
                worksheet.Cells[1, i] = data_grid_report.Columns[i - 1].HeaderText;
            }
            // storing Each row and column value to excel sheet  
            for (int i = 0; i < data_grid_report.Rows.Count - 1; i++)
            {
                for (int j = 0; j < data_grid_report.Columns.Count; j++)
                {
                    worksheet.Cells[i + 2, j + 1] = data_grid_report.Rows[i].Cells[j].Value.ToString();
                }
            }
        }
    }
